...
export function RequireAdmin({ component: Component }: { component: AdminComponent }) {
  const { loading, profile, isConfigured, error, signOut } = useAdminAuth();
  const [, setLocation] = useLocation();
  const isAuthenticated = Boolean(profile);

  useEffect(() => {
    if (!loading && !isAuthenticated && !error) setLocation("/admin/login");
  }, [error, isAuthenticated, loading, setLocation]);
  ...
  if (!isAuthenticated) return <div className="grid min-h-screen place-items-center"><LoaderCircle className="h-7 w-7 animate-spin text-cyan-600" /></div>;
  return <Component />;
}

...
export default function Home() {
  ...
  const consoleStyle = {
    "--rf-console-x": `${getSceneOffset(scrollProgress, 36)}px`,
    "--rf-console-y": `${getSceneOffset(scrollProgress, -28)}px`,
    "--rf-console-rotate": `${getSceneOffset(scrollProgress, -5)}deg`,
    "--rf-console-progress": scrollProgress,
  } as CSSProperties;
        ...
        <section className="rf-hero-frame relative mx-4 overflow-hidden rounded-[2.3rem] border border-white/70 bg-[#0a1113] text-white shadow-[0_30px_90px_-35px_rgba(8,24,31,.7)] sm:mx-6 lg:mx-8">
          ...
          <div className="relative mx-auto grid min-h-[690px] max-w-[1440px] items-center gap-10 px-6 py-16 sm:px-10 sm:py-20 lg:grid-cols-[.78fr_1.22fr] lg:px-16 lg:py-24">
            ...
            <div className="rf-payment-scene relative mx-auto min-h-[470px] w-full max-w-[700px] [perspective:1500px]" style={sceneStyle} aria-label="Interactive 3D recovery flow visualization">
              ...
              <div className="rf-scene-progress" aria-hidden="true"><span style={{ width: `${Math.max(12, scrollProgress * 100)}%` }} /></div>
              <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500"><span>Scroll to move the flow</span><span>{String(Math.max(1, Math.round(scrollProgress * 4))).padStart(2, "0")} — 04</span></div>
            </div>
          </div>
